...
namespace Lyricify.Lyrics.Helpers.General
{
    public static partial class StringHelper
    {
        ...
        {
            return Regexes.NumberRegex().IsMatch(str);
        }
        ...
        public static bool HasCJK(this string str, bool includeColon = false)
        {
            ...
            str = Regexes.SymbolRegex().Replace(str, "");
            var k = Regexes.CJKRegex().Matches(str);
            return k.Count > 0;
        }
        ...
        public static bool IsCJK(this string str, bool includeColon = false)
        {
            ...
            return Regexes.IsCJKUnifiedIdeographsRegex().IsMatch(str);
        }
        ...
        public static string OptimizeCJK(this string str)
        {
            ...
            foreach (char ch in str)
            {
                ...
                bool isSymbol = Regexes.SymbolRegex().IsMatch(chStr);
                bool isCJK = Regexes.IsCJKUnifiedIdeographsRegex().IsMatch(chStr);
        ...
        public static bool HasChinese(this string str)
        {
            ...
            return Regexes.ChineseRegex().IsMatch(str);
        }
        ...
        public static bool IsEmoji(char character, bool full = true)
        {
            if (full)
            {
                return Regexes.EmojiMatchOneRegex().IsMatch(character.ToString());
            }
        ...
        {
            return Regexes.EmojiMatchOneRegex().IsMatch(str);
        }

        #endregion Emoji


        private partial class Regexes
        {
            ...
            [GeneratedRegex(@"\p{IsCJKUnifiedIdeographs}|^[\u0800-\u4e00]|^[\u4e00-\u9fa5]|^[\uac00-\ud7ff]")]
            public static partial Regex CJKRegex();

            [GeneratedRegex(@"\p{IsCJKUnifiedIdeographs}")]
            public static partial Regex IsCJKUnifiedIdeographsRegex();

            [GeneratedRegex("[\\u4e00-\\u9fff]$")]
            public static partial Regex ChineseRegex();

            [GeneratedRegex(EmojiPattern)]
            public static partial Regex EmojiMatchOneRegex();

#else
            private static Regex _NumberRegex;
            public static Regex NumberRegex() => _NumberRegex ?? (_NumberRegex = new Regex("^\\d+$", RegexOptions.Compiled));

            private static Regex _SymbolRegex;
            public static Regex SymbolRegex() => _SymbolRegex ?? (_SymbolRegex = new Regex("[`~!@#$%^&*()+=|{}':;',\\[\\].<>/?~！@#￥%……&*（）——+|{}《》【】‘；：\"”“’。，、？]"));

            private static Regex _CJKRegex;
            public static Regex CJKRegex() => _CJKRegex ?? (_CJKRegex = new Regex(@"\p{IsCJKUnifiedIdeographs}|^[\u0800-\u4e00]|^[\u4e00-\u9fa5]|^[\uac00-\ud7ff]"));

            private static Regex _IsCJKUnifiedIdeographsRegex;
            public static Regex IsCJKUnifiedIdeographsRegex() => _IsCJKUnifiedIdeographsRegex ?? (_IsCJKUnifiedIdeographsRegex = new Regex(@"\p{IsCJKUnifiedIdeographs}"));
            
            private static Regex _ChineseRegex;
            public static Regex ChineseRegex() => _ChineseRegex ?? (_ChineseRegex = new Regex("[\\u4e00-\\u9fff]$"));

            private static Regex _EmojiMatchOneRegex;
            public static Regex EmojiMatchOneRegex() => _EmojiMatchOneRegex ?? (_EmojiMatchOneRegex = new Regex(EmojiPattern, RegexOptions.Compiled));
#endif
